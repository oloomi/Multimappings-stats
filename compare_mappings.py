__author__ = 'soloomi'

from collections import defaultdict
import re
import pysam


def are_alignments_different(alignments):
    # Check if a multi-read is mapped to different locations with different alignments
    prev_aln = alignments[0][2]  # the MD:Z field
    for aln in alignments[1:]:
        if aln[2] != prev_aln:
            return True
    return False

def compare_mappings(reference_name, benchmark_sam_file_name, best_match_file_name,
mapping_sam_file_name, mapping_bam_file_name, output_file_name):
    # A dictionary like: {read_id: [list of mappings CIGAR strings]}
    read_alignments_dict = defaultdict(list)

    not_mapped_reads = 0  # Number of reads that are not mapped to any location

    # Reading the SAM file and creating a dictionary of read_id : alignment
    with open(mapping_sam_file_name) as sam_file:
        for line in sam_file:
            # Skip header lines
            if line[0] == "@":
                continue
            fields = line.rstrip().split("\t")
            read_id = fields[0]  # QNAME: Query template NAME
            cigar = fields[5]  # CIGAR string (ie. alignment)
            pos = int(fields[3])  # 1-based leftmost mapping POSition
            md_z = fields[-2]   # Alignment
            # * means no alignment for a read
            if cigar != "*":
                # Store all alignments of a read
                read_alignments_dict[read_id].append((pos, cigar, md_z))
            else:
                not_mapped_reads += 1

    print("Number of reads not mapped:", not_mapped_reads)

    best_match_dict = defaultdict(list)

    with open(best_match_file_name) as sam_file:
        for line in sam_file:
            # Skip header lines
            if line[0] == "@":
                continue
            fields = line.rstrip().split("\t")
            read_id = fields[0]  # QNAME: Query template NAME
            cigar = fields[5]  # CIGAR string (ie. alignment)
            pos = int(fields[3])  # 1-based leftmost mapping POSition
            md_z = fields[-2]   # Alignment
            # * means no alignment for a read
            if cigar != "*":
                # Store all alignments of a read
                best_match_dict[read_id].append((pos, cigar, md_z))
            else:
                not_mapped_reads += 1

    print("Best-match number of reads not mapped:", not_mapped_reads)

    # The benchmark file contains the correct mapping location for each read
    # and is generated by the read simulator
    with open(benchmark_sam_file_name) as sam_file:
        with open(output_file_name, 'w') as out_file:
            line_num = 0
            for line in sam_file:
                # Skip header lines
                if line[0] == "@":
                    continue
                # Benchmark read specs
                fields = line.rstrip().split("\t")
                benchmark_read_id = fields[0]  # QNAME: Query template NAME
                benchmark_cigar = fields[5]  # CIGAR string (ie. alignment)
                benchmark_pos = int(fields[3])  # 1-based leftmost mapping POSition

                # Add pair suffix to read ID, because we have done single-end mapping on paired-end data
                # line_num += 1
                # if line_num % 2 == 0:
                #     benchmark_read_id += "/2"
                # else:
                #     benchmark_read_id += "/1"

                # * means no alignment for a read
                if benchmark_cigar != "*":
                    # We find the benchmark read in our mapping results (mapping synthetic reads)
                    # If it's a multi-read with different alignments
                    if benchmark_read_id in read_alignments_dict and len(read_alignments_dict[benchmark_read_id]) > 1 \
                            and are_alignments_different(read_alignments_dict[benchmark_read_id]):
                        # out_file.write("{}\t{}\n".format((benchmark_pos, benchmark_cigar), read_alignments_dict[benchmark_read_id]))
                        md_z_list = [aln[2] for aln in read_alignments_dict[benchmark_read_id]]
                        if not (benchmark_cigar == "150=" and "MD:Z:150" in md_z_list):
                            # Read_ID Correct_Mapping Multi-mappings
                            out_file.write("\n\n{}\n\n{}\t{}\n\n".
                                       format(benchmark_read_id, benchmark_pos, benchmark_cigar))
                            out_file.write("{}\n\n".format(best_match_dict[benchmark_read_id]))
                            for aln in read_alignments_dict[benchmark_read_id]:
                                out_file.write("{}\t{}\n".format(aln[0], aln[2][5:]))
                        #     (pos, cigar, md_z) = alignment
                        #     out_file.write("\nStatus at mapping location: {}\n".format(alignment))
                        #     snps = mdz_parser(md_z)
                        #     # For each SNP in that read alignment
                        #     for snp in snps:
                        #         # bases = bases_at_pos(mapping_bam_file_name, "gi|448814763|ref|NC_000962.3|", pos + snp[0] - 1)
                        #         # "gi|1050|ref|NC_Test_1|"
                        #         bases = bases_at_pos(mapping_bam_file_name, reference_name, pos + snp[0] - 1)
                        #         # 1-based
                        #         out_file.write("SNP at read position {} and reference position {}:\n".format(snp[0] + 1, pos + snp[0]))
                        #         out_file.write("{}\n".format(bases))

                    # else:
                    #     out_file.write("read not found!\n")
    print("Done!")


def bases_at_pos(file_name, chromosome, pos):
    results = []
    # BAM file coordinates are 0-based
    pos -= 1
    bam_file = pysam.AlignmentFile(file_name, 'rb')
    for pileup_column in bam_file.pileup(chromosome, pos, pos + 1, truncate=True):
        # print("\nThere are %s reads overlapping the coordinate at %s:%s" %
        #       (pileup_column.n, chromosome, pileup_column.pos + 1))
        coverage = pileup_column.n
        # Nucleotides at the query position for each read
        for pileup_read in pileup_column.pileups:
            if not pileup_read.is_del and not pileup_read.is_refskip:
                # base_qual = pileup_read.alignment.query_alignment_qualities[pileup_read.query_position]
                base = pileup_read.alignment.query_sequence[pileup_read.query_position]
                results.append(base)
                # print("Base {} with mapping quality score {}".format(base, base_qual))
    return results


def mdz_parser(mdz_str):
    """
    Takes the MD:Z filed string and returns a list of SNP positions and the SNP base
    :param mdz_str: the string from MD:Z field of the SAM file for a read
    :return: List of (pos, SNP_base) tuples. Please note that pos is 0-based;
    therefore, pos 0 points to the first base in the sequence.
    """

    # 'MD:Z:15A15A9G15C42'
    # Removing the MD:Z: prefix from the string
    mdz_str = mdz_str[5:]
    # Splitting into alphabetical and numerical parts
    mdz_parsed = filter(None, re.split(r'(\d+)', mdz_str))
    # mdz_parsed = re.findall(r"[^\W\d_]+|\d+", mdz_str)

    # Return list
    pos_snps = []
    pos = 0
    for item in mdz_parsed:
        if item.isalpha():
            for base in item:
                pos_snps.append((pos, base))
                pos += 1
        elif item.isnumeric():
            pos += int(item)
        else:
            print("ERROR: Invalid MD:Z field in SAM file: {}".format(item))

    return pos_snps
